...
async function preflight(): Promise<void> {
    try {
        await axios.get("https://ticker.finology.in", {
            headers: getHeaders(),
            withCredentials: true, // React Native axios respects this
        });
 ...
 */
export async function searchFinology(
    query: string
): Promise<any | null> {
    const url = `https://ticker.finology.in/GetSearchData.ashx?q=${encodeURIComponent(
        query
    )}`;
