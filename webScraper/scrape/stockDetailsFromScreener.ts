 ...
 *
 * NOTE:
 * - Uses axios + cheerio-without-node-native (RN-safe)
 * - No tough-cookie or axios-cookiejar-support (RN cookies auto-managed)
 * - Random delays + UA headers included for stealth
 */

import axios from "axios";
import cheerio from "cheerio-without-node-native" 
// import stockMapping from "../symbol-code-mapping/stock_mapping.json"; // if you have mapping JSON
...
// === Core Scraper ===
export async function stockDetailsFromScreener(
    ticker: string
): Promise<any | null> {
    try {
        ...
        let resp: any = null;
        try {
            resp = await axios.get(baseUrl + ticker, { headers: defaultHeaders });
        } catch {
            try {
                const comArr = await searchFinology(ticker);
                const code = comArr[0]?.FINCODE;
                if (code) {
                    resp = await axios.get(baseUrl + `SCRIP-${code}`, {
                        headers: defaultHeaders,
                    });
                }
            } catch {
        ...
        return {
            quarters: currentFinQuarters,
            quarterlySales: quarterlyData.quarterlySales.slice(sliceLength),
            quarterlyPat: quarterlyData.quarterlyPat.slice(sliceLength),
            quarterlyEps: quarterlyData.quarterlyEps.slice(sliceLength),
            quarterlyOpProfit: quarterlyData.quarterlyOpProfit.slice(sliceLength),
            years,
